...
项目按需渲染规则，但不会在同一数据版本内反复转换：例如第一次请求
`/geosite/google` 时生成内容并放入进程内存，之后的相同请求直接复用；上游数据或正则
模式热载入后会创建新的不可变数据快照，新快照会在首次请求时重新生成。响应另外带有
`Cache-Control: public, max-age=3600, stale-while-revalidate=86400` 和 `ETag`，因此 Surge
